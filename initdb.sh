...
if ! grep -q "wal_level = logical" "$CONF_FILE"; then
  echo "⚙️  Configuring PostgreSQL for logical replication..."
  cat >>"$CONF_FILE" <<EOF
wal_level = logical
max_wal_senders = 10
...
EOF
  ...
  echo "ℹ️  Replication already configured."
fi
...
if [[ -f "/docker-entrypoint-initdb.d/dhis-backup.gz" ]]; then
  ...
  echo "⚠️  Backup archive not found: /docker-entrypoint-initdb.d/dhis-backup.gz"
  exit 1
fi
